...
namespace _05._Pizza_Calories
{
	public class Dough
	{
		...
		{
			object ftObject = allowedFlourTypes.First(ft => (string)ft.GetType().GetProperty("FlourType").GetValue(ft, null) == this.FlourType.ToLower());
			double ftModifier = (double)ftObject.GetType().GetProperty("CaloriesModifier").GetValue(ftObject, null);

			object btObject = allowedBakingTechniqueTypes.First(bt => (string)bt.GetType().GetProperty("BakingTechnique").GetValue(bt, null) == this.BakingTechnique.ToLower());
			double btModifier = (double)btObject.GetType().GetProperty("CaloriesModifier").GetValue(btObject, null);
